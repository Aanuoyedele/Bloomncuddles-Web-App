...
// Game content for each game type - 20 questions each
const GAME_CONTENT: Record<string, any> = {
    ...
    'quiz': {
        'Sight Words Race': [
            ...
            { question: 'Which word says "PLAY"?', options: ['YALP', 'PLAY', 'PAYЛ', 'LYAP'], correctIndex: 1 },
    ...
    'drag_drop': {
        'Shape Sorter': [
            ...
            { question: 'Which shape has 2 long sides and 2 short sides?', options: ['Square', 'Rectangle', 'Circle', 'Triangle'], correctIndex: 1 },
...
export default function GamePlayerPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        const fetchGame = async () => {
            try {
                const data = await api.get(`/games/${resolvedParams.id}`);
                setGame(data);
            } catch (err) {
    ...
    // Initialize shuffled questions when game loads
    useEffect(() => {
        if (game) {
    ...
    // Handle quiz/multiple choice answer
    ...
    const resetGame = () => {
        // Re-shuffle questions for replay
        const allQuestions = getQuestions();
        setShuffledQuestions(shuffleArray(allQuestions));
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary/5 to-secondary/5">
                <div className="text-center">
                    ...
                    <button onClick={() => router.push('/dashboard/games')} className="mt-4 bg-primary text-white font-bold px-[15px] py-[15px] rounded-[6px]">
                        Back to Games
                    </button>
                </div>
            </div>
    ...
    const renderGame = () => {
        // QUIZ / DRAG_DROP / LOGIC - Multiple choice
